...
def main():
    ...
    log_dir = TENSORBOARD_DIR / f"train_conditional_{timestamp}"
    ...
    dummy_c = torch.rand(1, 5).to(device)
    try:
        writer.add_graph(model, (dummy_input, dummy_t, dummy_c))
    ...
    for epoch in range(TRAIN_CONFIG["n_epoch"]):
        ...
        if epoch % TRAIN_CONFIG["save_interval"] == 0 or epoch == TRAIN_CONFIG["n_epoch"] - 1:
            save_path = WEIGHTS_DIR / f"context_model_{epoch}.pth"
            torch.save(model.state_dict(), save_path)
            print(f"模型已保存: {save_path}")
            
            if avg_loss < best_loss:
                best_loss = avg_loss
                best_path = WEIGHTS_DIR / "context_model_best.pth"
